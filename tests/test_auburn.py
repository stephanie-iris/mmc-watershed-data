...
class AuburnTests(unittest.TestCase):
    def test_collect_station_writes_raw_and_processed_csv(self) -> None:
        station = Station(
            "auburn", "Auburn", "lake_ogletree", "Lake Ogletree", "channel-id"
        )
        payload = {
            "value": {
                "records": [
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "timestamp_ms,value", result.raw_csv_path.read_text(encoding="utf-8")
            )
            self.assertIn(
                "Date_hour,RainIn", result.processed_path.read_text(encoding="utf-8")
            )
    ...
    def test_invalid_payload_keeps_raw_json_evidence(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            processed_dir = root / "processed"
            with (
                patch("mmc_watershed_data.auburn.post_json", return_value=payload),
                patch("mmc_watershed_data.auburn.time_module.sleep"),
            ):
